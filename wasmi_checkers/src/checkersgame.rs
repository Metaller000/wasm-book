...
fn load_instance(import_resolver: &impl ModuleImportResolver, module_file: &str) -> Result<ModuleRef> {
...
impl CheckersGame {
    ...
    pub fn init(&mut self) {
        self.module_instance.invoke_export("initBoard", &[], &mut self.runtime).unwrap();
    }
    ...
    pub fn move_piece(&mut self, from: &Coordinate, to: &Coordinate) -> Result<bool> {
        let res = self.module_instance.invoke_export("move", &[
            ...
            RuntimeValue::from(to.1)], &mut self.runtime, )?;
    ...
    pub fn get_board_contents(&self) -> String{
  ...
  .---.---.---.---.---.---.---.---."#;
    ...
    pub fn display_board(&mut self) {
        let board_display = self.get_board_contents();
        println!("\nGame board:{}\n", board_display);
        println!("Current turn is : {:?}", self.get_turn_owner().unwrap());
    }
}
