...
fn main() -> Result<(), Box<Error>> {
    ...
    {
        let mut f = File::open("../fundamentals/add2.wasm")?;
    ...
    let result: Result<Option<RuntimeValue>, wasmi::Error> = instance.invoke_export("add", &args, &mut NopExternals);
    ...
    match result {
        ...
        Err(err) => {
            println!("Error invoking wasm export {}", err);
        }
    }
